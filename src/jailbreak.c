...
compatibility_t compatible_devices[] = { { "N81AP", "10B400" },

{ "N41AP", "10B350" }, { "N42AP", "10B350" },

{ "N94AP", "10B329" },

{ "N90BAP", "10B329" }, { "N90AP", "10B329" }, { "N92AP", "10B329" }, { "N81AP",
		"10B329" }, { "N88AP", "10B329" },

{ "N78AP", "10B329" },

{ "N41AP", "10B329" }, { "N42AP", "10B329" },

{ "J1AP", "10B329" }, { "J2AP", "10B329" }, { "J2aAP", "10B329" },

{ "P101AP", "10B329" }, { "P102AP", "10B329" }, { "P103AP", "10B329" },

{ "K93AP", "10B329" }, { "K93AAP", "10B329" }, { "K94AP", "10B329" }, { "K95AP",
		"10B329" },

{ "P105AP", "10B329" }, { "P106AP", "10B329" }, { "P107AP", "10B329" },
...
static void cb2(am_device_notification_callback_info * info, void *foo) {
	...
	if (info->msg == ADNCI_MSG_CONNECTED) {
		...
		CFStringRef product = AMDeviceCopyValue(dev, 0,
		...
		assert(
				!AMDeviceStartService(dev, CFSTR("com.apple.afc"), &afc_socket, NULL));
		...
		assert(
				!AMDeviceStartService(dev, CFSTR("com.apple.mobile.mobile_image_mounter"), &mim_socket1, NULL));
		assert(mim_socket1);
		...
		CFDictionarySetValue(dict, CFSTR("ImagePath"),
				CFSTR("/var/mobile/Media/PublicStaging/staging.dimage"));
		...
		CFDictionarySetValue(dict, CFSTR("ImageSignature"),
				CFDataCreateWithBytesNoCopy(NULL, sig, sizeof(sig),
						kCFAllocatorNull));
		...
		if (ddi_dmg) {
			...
			assert(
					!AFCRenamePath(afc, "PublicStaging/ddi.dimage",
		...
		if (strstr(CFDataGetBytePtr(CFPropertyListCreateXMLData(NULL, result)),
				"ImageMountFailed")) {
			timesl += 100;
			goto Retry;
		}
...
int jailbreak_device(const char *uuid, status_cb_t cb) {
	...
	char *bargv[] = { "idevicebackup2", "backup", backup_dir,
	NULL };
	...
	/* Reboot for the sake of posterity. Gets rid of all Developer images mounted. */
	{
		...
		if (backup_symlink(backup, "MediaDomain", "Media/Recordings/.haxx",
				"/var/db/launchd.db/com.apple.launchd", 501, 501, 4) != 0) {
		...
		if (backup_add_file_from_path(backup, "MediaDomain",
				"payload/common/overrides.plist",
				"Media/Recordings/.haxx/overrides.plist", 0100755, 0, 0, 4)
				!= 0) {
	...
	if (stat("payload/iOSUpdaterHelper.dmg", &st)) {
		ZipInfo* info =
				PartialZipInit(
						"http://appldnld.apple.com/iOS6/041-8518.20121029.CCrt9/iOSUpdater.ipa");
		if (!info) {
			ERROR("Cannot make PartialZip context\n");
			return -1;
		}
		PartialZipSetProgressCallback(info, callback);
		CDFile* file = PartialZipFindFile(info,
				"Payload/iOSUpdater.app/iOSUpdaterHelper.dmg");
		if (!file) {
	...
	if (stat("payload/iOSUpdaterHelper.dmg.signature", &st)) {
		...
		PartialZipSetProgressCallback(info, callback);
		CDFile* file = PartialZipFindFile(info,
				"Payload/iOSUpdater.app/iOSUpdaterHelper.dmg.signature");
		if (!file) {
	 ...
	 */
	{
		...
		if (backup_mkdir(backup, "MediaDomain",
				"Media/Recordings/.haxx/var/untether", 0755, 0, 0, 4) != 0) {
			ERROR("Could not make folder\n");
			return -1;
		}

		{
			...
			snprintf(launchd_conf_path, 128, "payload/common/launchd.conf",
					build, product);

			if (backup_add_file_from_path(backup, "MediaDomain",
					launchd_conf_path,
					"Media/Recordings/.haxx/var/untether/launchd.conf", 0100644,
					0, 0, 4) != 0) {
			...
			if (backup_symlink(backup, "MediaDomain",
					...
					"/private/var/untether/launchd.conf", 0, 0, 4) != 0) {
				ERROR("Failed to symlink launchd.conf!\n");
				return -1;
			}
			if (backup_add_file_from_path(backup, "MediaDomain",
					"payload/common/tar",
					"Media/Recordings/.haxx/var/untether/tar", 0100755, 0, 0, 4)
					!= 0) {
				ERROR("Could not add tar");
				return -1;
			}
			...
			if (backup_symlink(backup, "MediaDomain",
					...
					"/private/var/untether/tar", 0, 0, 4) != 0) {
				ERROR("Failed to symlink tar!\n");
				return -1;
			}
			if (backup_symlink(backup, "MediaDomain",
					"Media/Recordings/.haxx/usr/libexec/dirhelper",
					"/private/var/untether/dirhelper", 0, 0, 4) != 0) {
				ERROR("Failed to symlink dirhelper!\n");
				return -1;
			}
			if (backup_add_file_from_path(backup, "MediaDomain",
					"payload/common/install.deb",
					"Media/Recordings/.haxx/var/untether/install.deb", 0100755,
					0, 0, 4) != 0) {
				ERROR("Could not add dirhelper");
				return -1;
			}
			if (backup_add_file_from_path(backup, "MediaDomain",
					"payload/common/dirhelper",
					"Media/Recordings/.haxx/var/untether/dirhelper", 0100755, 0,
					0, 4) != 0) {
				ERROR("Could not add dirhelper");
				return -1;
			}
			if (backup_add_file_from_path(backup, "MediaDomain", jb_path,
					"Media/Recordings/.haxx/var/untether/untether", 0100755, 0,
					0, 4) != 0) {
				ERROR("Could not add jb");
				return -1;
			}
			if (backup_add_file_from_path(backup, "MediaDomain", amfi_path,
					"Media/Recordings/.haxx/var/untether/_.dylib", 0100644, 0,
					0, 4) != 0) {
			...
			if (backup_add_file_from_path(backup, "MediaDomain",
					"payload/Cydia.tar",
					"Media/Recordings/.haxx/var/untether/Cydia.tar", 0100644, 0,
					0, 4) != 0) {
	...
	if (is_jailbroken) {
		cb(
				"Your device is now jailbroken, it is now preparing to reboot automatically.\n",
				100);
		WARN(
				"Your device is now jailbroken, it is now preparing to reboot automatically.\n");
		...
		lockdown_start_service(lockdown, "com.apple.mobile.diagnostics_relay",
				&diag_port);

		desc.port = diag_port;
		if (diagnostics_relay_client_new(device->client, &desc,
				&diagnostics_client) == DIAGNOSTICS_RELAY_E_SUCCESS) {
			diagnostics_relay_restart(diagnostics_client, 0);
		}

	} else {
		cb(
				"Your device has encountered an error during the jailbreak process, unplug it and try again.\n",
				100);
		WARN(
				"Your device has encountered an error during the jailbreak process, unplug it and try again.\n");
	}
